...
func (es *elasticsearch) SearchRequest(indexNames []string, quer *entity.QueryDocs) (*elastic.SearchResult, error) {
	...
	if got, wa:= res.TotalHits(), int64(1); got != want {
		err = errors.New("expected SearchResult.TotalHits() = %d; got %d", want, got)
		return nil,err
	}
	if got, want := len(res.Hits.Hits), 1; got != want {
		err = errors.New("expected len(SearchResult.Hits.Hits) = %d; got %d", want, got)
		return nil,err
	}
	return res, nil
}
