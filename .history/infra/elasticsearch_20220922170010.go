...
if res == nil {
		common.Logg.Fatal("expected results != nil; got nil")
	}
	if res.Hits == nil {
		common.Logg.Fatal("expected SearchResult.Hits != nil; got nil")
	}
	if got, want := res.TotalHits(), int64(1); got != want {
		common.Logg.Fatalf("expected SearchResult.TotalHits() = %d; got %d", want, got)
	}
	if got, want := len(res.Hits.Hits), 1; got != want {
		common.Logg.Fatalf("expected len(SearchResult.Hits.Hits) = %d; got %d", want, got)
	}
	return res, nil
}
