...
func (es *elasticsearch) SearchRequest(indexNames []string, quer *entity.QueryDocs) (*elastic.SearchResult, error) {
	...
	if res == nil {
		errors.New("expected results != nil; got nil")
		return nil,
	}
	if res.Hits == nil {
		errors.New("expected SearchResult.Hits != nil; got nil")
		return nil,
	}
	if got, want := res.TotalHits(), int64(1); got != want {
		errors.New("expected SearchResult.TotalHits() = %d; got %d", want, got)
		return nil,
	}
	if got, want := len(res.Hits.Hits), 1; got != want {
		errors.New("expected len(SearchResult.Hits.Hits) = %d; got %d", want, got)
		return nil,
	}
	return res, nil
}
