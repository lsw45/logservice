...
func (es *elasticsearch) SearchRequest(indexNames []string, quer *entity.QueryDocs) (*elastic.SearchResult, error) {
	...
	if res == nil {
		common.Logger.Fatal("expected results != nil; got nil")
	}
	if res.Hits == nil {
		common.Logger.Fatal("expected SearchResult.Hits != nil; got nil")
	}
	if got, want := res.TotalHits(), int64(1); got != want {
		common.Logger.Fatalf("expected SearchResult.TotalHits() = %d; got %d", want, got)
	}
	if got, want := len(res.Hits.Hits), 1; got != want {
		common.Logger.Fatalf("expected len(SearchResult.Hits.Hits) = %d; got %d", want, got)
	}
	return res, nil
}
